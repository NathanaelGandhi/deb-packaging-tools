...
from .common import check_and_install_dpkg_deps
from .common import check_py_deps
from .common import env_args_list_to_dict
...
def _determine_package_name_version(cwd, package_name_version: str = None):
    ...
    if _is_valid_debian_package_name_version(package_name_version) == False:
        raise ValueError(
            "Package name and version must be in the format of <package>_<version>"
        )
...
def _dh_make_generator(
    package_name_version: str, cwd=None, env=None, check=True, upstream_source=True
):
    check_and_install_dpkg_deps(["dh-make"])
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.mode == "dh_make":
        _dh_make_generator(
            _determine_package_name_version(args.cwd, args.p),
            cwd=args.cwd,
            env=env_args_list_to_dict(args.env),
            upstream_source=not args.no_upstream_source,
        )
